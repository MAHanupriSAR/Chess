...
vector<Move> generateAllMoves(BitBoard& bitBoard) {
    vector<Move> moveList;
    moveList.reserve(35);

    int startPiece = bitBoard.turn ? 6 : 0;
    int endPiece   = bitBoard.turn ? 12 : 6;

    for (int p = startPiece; p < endPiece; p++) {
        uint64_t pieceBoard = bitBoard.board[p];
        ...
        while (pieceBoard) {
            ...
            uint64_t movesBitboard = bitBoard.getMoves(r, c);
...
int minimax(BitBoard bitBoard, int depth, int alpha, int beta){
    if(depth == 0){
        return evaluate(bitBoard);
    }

    vector<Move> moves = generateAllMoves(bitBoard);
    if (moves.empty()) {
        if (bitBoard.isCheck()) {
...
Move searchBestMove(BitBoard bitBoard, int depth){
    vector<Move> moves = generateAllMoves(bitBoard);
    Move bestMove;
    int bestVal = -INF;
    int alpha = -INF;
...
Move getBestMove(string fenString, string selfPieceColor, int castlingRights, int enPassantTarget, int depth){
    BitBoard bitBoard(fenString, selfPieceColor, 1, castlingRights, enPassantTarget);
    return searchBestMove(bitBoard, depth); 
}
